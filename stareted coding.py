...
for Gamma in num.arange(0.01,1.00,0.05):
    ...
    for Th in range(0,100,1):
        for Thc in range(Th,100,1):     ###ya range (Th,100,1)
            sigma1=0
            sigma2=0
            ...
            for Pa in range (0,10,1):
                for Poc in range (0,10,1):
                    if(Pa+Poc<10):
                        for Pc in range (0,400,1):
                            for Pos in range (0,300,1):
...
lns1 = ax1.plot(Gammalist, Thfinallist , '-r', marker='x', label=r'$K_{h}$')
lns2 = ax1.plot(Gammalist, Thcfinallist , '-r', marker='>', label=r'$K_{hc}$')
lns3 = ax2.plot(Gammalist, Pcfinallist , '-b', marker='<', label=r'$P_{T}$')
lns4 = ax2.plot(Gammalist, Posfinallist , '-b', marker='D', label=r'$P_{cp}^{\ s}$')
lns5 = ax2.plot(Gammalist, Pafinallist , '-b', marker='.', label=r'$P_{A}$')
lns6 = ax2.plot(Gammalist, Pocfinallist , '-b', marker='|', label=r'$P_{cp}^{\ c}$')
...
ax1.set_xlabel('s')
ax1.set_ylabel('Caching Threshold', color='r')
